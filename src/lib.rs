...
mod tests {
   ...
   777: [
	...
	fn decode_to_test() {
		...
		let dict: HashMap<u64, HashMap<String, String>> = crate::decode_to(TEST_DATA_DEFINITE.iter()).unwrap().unwrap();
		assert_eq!(1, dict.len());
		assert!(dict.contains_key(&555));
		let map2 = dict.get(&555).unwrap();
		assert_eq!(2, map2.len());
		...
		let dict: BTreeMap<i64, Vec<i64>> = crate::decode_to(TEST_DATA_DEFINITE.iter()).unwrap().unwrap();
		let arr = dict.get(&777).unwrap();
		assert_eq!(2, arr.len());
		assert_eq!(11, arr[0]);
		assert_eq!(-22, arr[1]);
